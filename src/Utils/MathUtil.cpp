...
//标准差σ=sqrt(s^2)
double MathUtil::deviation(const float* ts, int start, int end) {
    int len = end - start;
    double sum=0;
    for(int i=start; i < end; ++i)
        sum += ts[i];
    return deviation(ts, start, end, sum / len);
}
...
double MathUtil::avg(const float *timeSeries, int len) {
    return avg(timeSeries, 0, len);
}
...
double MathUtil::avg(const float* timeSeries, int start, int end) {
    double sum = 0;
    for(int i=start; i< end;++i)
        sum += timeSeries[i];
    return sum / (end - start);
...
double MathUtil::deviation(const float *timeSeries, int len) {
    return deviation(timeSeries, 0, len);
}

float* MathUtil::z_Normalize(const float* timeSeries, int len) {
    auto* ret = new float[len];
    double mean = avg(timeSeries, len);
    double std = deviation(timeSeries, len);
    if (std < 1e-7)
        std = 1;
    for (int i = 0; i < len; i++) {
        ret[i] = (float) ((timeSeries[i] - mean) / std);
    }
    return ret;
}
...
int MathUtil::bitDiffNum(int i, int j){
    int differentBits = i ^ j;
    int res = 0;
...
double MathUtil::errorRatio(vector<PqItemSeries *> &approx,
  ...
  for (int i = 0; i < approx.size(); ++i) {
    if (approx[i]->dist == 0 && exact[i]->dist == 0) {
      sum += 1;
    } else if (approx[i]->dist == 0 || exact[i]->dist == 0) {
      cnt++;
    } else {
      sum += (sqrt(approx[i]->dist) / sqrt(exact[i]->dist));
    }
  }
  return sum / (approx.size() - cnt);
}
